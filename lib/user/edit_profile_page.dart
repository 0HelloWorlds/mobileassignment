...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  Future<void> _save() async {
    ...
    try {
      final contactsResponse = await supabase.functions.invoke('get-emergency-contacts');
      final existingContacts = contactsResponse.data['contacts'] as List;

      final duplicate = existingContacts.any(
      ...
      final updateResponse = await supabase.functions.invoke('update-user', body: {
        'name': _nameController.text.trim(),
        'phone_number': newPhoneNumber,
      });

      if (updateResponse.data['error'] != null) {
        throw Exception(updateResponse.data['error']);
      }

      if (_selectedPhotoBytes != null) {
        await uploadProfilePhoto(_selectedPhotoBytes!, userId);
      }
