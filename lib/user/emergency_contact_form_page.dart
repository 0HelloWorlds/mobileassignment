  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final duplicate = contacts.any((contact) {
        ...
        return contact['contact_number'].toString().trim() == emergencyNumber;
      });
      ...
      if (_isEditing) {
        final updateResponse = await supabase.functions.invoke('update-emergency-contact', body: {
          'contact_id': widget.existing!.contactId,
        ...
        if (updateResponse.data['success'] != true) {
          throw Exception(
            updateResponse.data['error'] ?? 'Failed to update emergency contact.',
          );
        }
      }
      else {
        final addResponse = await supabase.functions.invoke('create-emergency-contact', body: {
          'full_name': _nameController.text.trim(),
          'contact_number': emergencyNumber,
          'relationship': _relationshipController.text.trim(),
        });
        if (addResponse.data['success'] != true) {
          throw Exception(
            addResponse.data['error'] ?? 'Failed to add emergency contact.',
          );
        }
      }
